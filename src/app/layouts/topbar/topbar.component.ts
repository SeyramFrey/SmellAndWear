...
import { allNotification } from './data'
...
export class TopbarComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.adminAuthService.user$.pipe(
      takeUntil(this.destroy$)
    ).subscribe(user => {
      this.userEmail = user?.email || null;
    });

    this.adminAuthService.isAdmin$.pipe(
      takeUntil(this.destroy$)
    ).subscribe(isAdmin => {
    ...
    if (val.length === 0) {
      if (this.flagvalue === undefined) { this.valueset = 'assets/images/flags/french.svg'; }
    } else {
    ...
    this.cart_length = this.cartData?.length || 0;
...
*/
  ...
  */
  ...
  /**
   * Logout the admin user using AdminAuthService
   * Enhanced with comprehensive cleanup and error handling
   */
  async logout(): Promise<void> {
    console.log('[TopbarComponent] Initiating admin logout...');
    
    try {
      // Sign out via AdminAuthService
      await this.adminAuthService.signOut();
      
      console.log('[TopbarComponent] Admin logged out successfully');
      this.completeLogoutCleanup();
      this.navigateToLogin('admin');
    } catch (error: unknown) {
      console.error('[TopbarComponent] Error during logout:', error);
      // Force cleanup even if logout fails
      this.forceLogoutCleanup();
      this.navigateToLogin('admin');
    }
  }

  /**
   * Complete logout cleanup for successful logouts
   */
  ...
  /**
   * Force cleanup when logout service fails
   */
  private forceLogoutCleanup(): void {
    try {
      // Force clear all session storage
   ...
   */
  private navigateToLogin(userType: 'admin' | 'user' | 'default' | 'error'): void {
    const currentUrl = this.router.url;
    
    // Determine redirect URL based on user type and current location
    if (userType === 'admin' || currentUrl.startsWith('/admin')) {
    ...
    console.log(`Redirected to login (type: ${userType})`);
  }
